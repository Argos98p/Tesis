...
export const SalesValueWidget = (props) => {
    const { title, value, percentage,placeId,placeNombre } = props;
    const percentageIcon = percentage < 0 ? faAngleDown : faAngleUp;
    const percentageColor = percentage < 0 ? "text-danger" : "text-success";
    ...
    useEffect(() => {
        axios.get(`http://localhost:8083/place/${placeId}/comentarios`).then((resp)=> {
            if (resp.status === 200) {

                setListaComentarios(resp.data);
        ...
        <Card className="bg-secondary-alt shadow-sm">
            <Card.Header className="d-flex flex-row align-items-center flex-0">
                <div className="d-block">
                    ...
                    <h3>Visitas : {listaComentarios.length}</h3>
            ...
            <Card.Body className="p-2">
                <SalesValueChart  listaComentarios={listaComentarios}/>
            </Card.Body>
        </Card>
    );
};
